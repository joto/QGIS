...
void QgsDecorationTileGrid::drawTileCoordinates( QgsRenderContext &context, int zoom ) const
{
  ...
  if ( mGridAnnotationStyle == GridAnnotationStyle::Border )
  {
    ...
    QgsTextRenderer::drawText( QRectF( borderWidth, borderWidth, 100, 100 ), 0, Qgis::TextHorizontalAlignment::Left, outputStringList, context, mTextFormat );
    ...
    for ( int c = range.startColumn(); c <= range.endColumn(); ++c )
    {
      ...
      if ( extent.width() >= minSizeToDraw )
      {
        ...
        QgsTextRenderer::drawText( extent.toRectF(), 0, Qgis::TextHorizontalAlignment::Center, outputStringList, context, mTextFormat, true, Qgis::TextVerticalAlignment::Top );
      }
    }
    ...
    for ( int r = range.startRow(); r <= range.endRow(); ++r )
    {
      ...
      if ( extent.height() >= minSizeToDraw )
      {
        ...
        QgsTextRenderer::drawText( extent.toRectF(), 0, Qgis::TextHorizontalAlignment::Left, outputStringList, context, mTextFormat, true, Qgis::TextVerticalAlignment::VerticalCenter );
      }
    }
  }
  else if ( mGridAnnotationStyle == GridAnnotationStyle::Center )
  {
    for ( int c = range.startColumn(); c <= range.endColumn(); ++c )
    {
      for ( int r = range.startRow(); r <= range.endRow(); ++r )
      {
        ...
        QgsTextRenderer::drawText( center, 0, Qgis::TextHorizontalAlignment::Center, annotationStringList, context, mTextFormat );
      }
    }
  }
}
